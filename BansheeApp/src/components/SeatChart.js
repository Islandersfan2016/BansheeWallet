...
const SeatChart = ({ occasion, ticketMarketplace, provider, setToggle }) => {
  const [seatsTaken, setSeatsTaken] = useState(false)
  const [hasSold, setHasSold] = useState(false)

  const getSeatsTaken = async () => {
    const seatsTaken = await ticketMarketplace.getSeatsTaken(occasion.id)
  ...
  const buyHandler = async (_seat) => {
    ...
    const transaction = await ticketMarketplace.connect(signer).mint(occasion.id, _seat, { value: occasion.cost })
